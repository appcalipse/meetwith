...
export const ConnectModal: React.FC = ({}) => {
  const { isConnectionOpened, closeConnection, redirectPath , shouldRedirect} = useContext(
  ...
  const doLogin = async (wallet: Wallet) => {
    if (
      ...
      await handleLogin(wallet, undefined, undefined,shouldRedirect, redirectPath)
    }
  }
