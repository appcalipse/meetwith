...
import ContactRejectInviteModal from '../contact/ContactRejectInviteModal'
...
const Contact: React.FC<{ currentAccount: Account }> = ({ currentAccount }) => {
  ...
  const { requestCount } = useContext(ContactCountStateContext)
    ...
    <ContactStateProvider>
      <Flex direction={'column'} maxWidth="100%">
        ...
        <ContactRejectInviteModal
          ...
          onClose={closeRejectModal}
          syncReject={address =>
            setRequests(requests.filter(r => r.address !== address))
          }
        />
        <HStack
