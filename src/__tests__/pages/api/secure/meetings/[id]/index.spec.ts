...
describe('/api/secure/meetings/[id]', () => {
  ...
  describe('POST - Update Meeting', () => {
    ...
    it('should return 409 for time not available error', async () => {
      mockGetMeetingFromDB.mockResolvedValue({ account_address: '0x1234567890abcdef' })
      mockGetAccountFromDB.mockResolvedValue({ address: '0x1234567890abcdef' })
      mockUpdateMeeting.mockRejectedValue(new TimeNotAvailableError('Time slot taken'))
    ...
    it('should return 412 for meeting creation error', async () => {
      mockGetMeetingFromDB.mockResolvedValue({ account_address: '0x1234567890abcdef' })
      mockGetAccountFromDB.mockResolvedValue({ address: '0x1234567890abcdef' })
      mockUpdateMeeting.mockRejectedValue(new MeetingCreationError('Creation failed'))
    ...
    it('should return 417 for meeting change conflict error', async () => {
      mockGetMeetingFromDB.mockResolvedValue({ account_address: '0x1234567890abcdef' })
      mockGetAccountFromDB.mockResolvedValue({ address: '0x1234567890abcdef' })
      mockUpdateMeeting.mockRejectedValue(new MeetingChangeConflictError('Conflict'))
    ...
    it('should return 405 for gate condition error', async () => {
      mockGetMeetingFromDB.mockResolvedValue({ account_address: '0x1234567890abcdef' })
      mockGetAccountFromDB.mockResolvedValue({ address: '0x1234567890abcdef' })
      mockUpdateMeeting.mockRejectedValue(new GateConditionNotValidError('Gate failed'))
    ...
    it('should return 404 for meeting session not found', async () => {
      mockGetMeetingFromDB.mockResolvedValue({ account_address: '0x1234567890abcdef' })
      mockGetAccountFromDB.mockResolvedValue({ address: '0x1234567890abcdef' })
      mockUpdateMeeting.mockRejectedValue(new MeetingSessionNotFoundError('Session not found'))
    ...
    it('should return 400 for transaction required error', async () => {
      mockGetMeetingFromDB.mockResolvedValue({ account_address: '0x1234567890abcdef' })
      mockGetAccountFromDB.mockResolvedValue({ address: '0x1234567890abcdef' })
      mockUpdateMeeting.mockRejectedValue(new TransactionIsRequired('Transaction needed'))
  ...
  describe('DELETE - Remove Meeting', () => {
    ...
    it('should return 409 for time conflict errors', async () => {
      mockGetAccountFromDB.mockResolvedValue({ address: '0x1234567890abcdef' })
      mockDeleteMeetingFromDB.mockRejectedValue(new TimeNotAvailableError('Conflict'))
