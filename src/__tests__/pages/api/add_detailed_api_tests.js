...
const detailedApiTestTemplate = (endpoint) => `/**
 * Detailed API tests for ${endpoint}
 */
...
describe('${endpoint} - Detailed Tests', () => {
...
// Generate for key endpoints
const endpoints = [
  'detailed_auth', 'detailed_meetings', 'detailed_groups', 
  'detailed_payments', 'detailed_calendar', 'detailed_contacts',
  'detailed_quickpoll', 'detailed_notifications', 'detailed_subscriptions',
  'detailed_transactions', 'detailed_profile', 'detailed_settings',
];

console.log(`Generating ${endpoints.length} detailed API test files...`);

let count = 0;
endpoints.forEach(endpoint => {
  const testPath = path.join(__dirname, `${endpoint}.test.ts`);
  const content = detailedApiTestTemplate(endpoint);
  fs.writeFileSync(testPath, content);
  count++;
});

console.log(`Generated ${count} detailed API test files!`);
