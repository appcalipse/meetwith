...
describe('WebDAVEventMapper', () => {
  ...
  describe('toUnified', () => {
    ...
    it('should map attendees from string array format', () => {
      const webdavEvent: WebDAVEvent = {
        ...
        rrule: null,
        attendees: [
          ['mailto:attendee1@example.com', 'CN=Attendee One', 'PARTSTAT=ACCEPTED', 'ROLE=CHAIR'],
          ['mailto:attendee2@example.com', 'CN=Attendee Two', 'PARTSTAT=DECLINED'],
        ],
      }
    ...
    it('should extract email from simple string format', () => {
      ...
      expect(result.attendees[0].email).toBe('simple@example.com')
    })
    ...
    it('should map event status correctly', () => {
      const confirmedEvent: WebDAVEvent = {
        uid: 'event1',
      ...
      const tentativeEvent: WebDAVEvent = {
        uid: 'event2',
      ...
      const cancelledEvent: WebDAVEvent = {
        uid: 'event3',
    ...
    it('should preserve WebDAV-specific provider data', () => {
      ...
      expect(result.providerData?.webdav?.duration).toEqual({ hours: 1, minutes: 30 })
    })
    ...
    it('should handle attendees with CUTYPE', () => {
      ...
      expect(result.attendees[0].providerData?.webdav?.cutype).toBe('INDIVIDUAL')
      expect(result.attendees[0].providerData?.webdav?.rsvp).toBe(true)
    })
  ...
  describe('fromUnified', () => {
    ...
    it('should convert attendees correctly', () => {
      ...
      expect(result.attendees).toHaveLength(2)
      expect(result.attendees[0]).toContain('mailto:attendee1@example.com')
      expect(result.attendees[0]).toContain('CN="Attendee One"')
      expect(result.attendees[0]).toContain('PARTSTAT=ACCEPTED')
      expect(result.attendees[0]).toContain('ROLE=CHAIR')
      expect(result.attendees[1]).toContain('PARTSTAT=TENTATIVE')
      expect(result.attendees[1]).toContain('ROLE=REQ-PARTICIPANT')
    })
    ...
    it('should map unified status to WebDAV status', () => {
      const baseEvent: Partial<UnifiedEvent> = {
        ...
        lastModified: new Date(),
        isReadOnlyCalendar: false,
        providerData: { webdav: { url: 'https://caldav.server.com/event123' } },
      }
    ...
    it('should handle attendee without name', () => {
      ...
      expect(result.attendees[0]).toContain('mailto:attendee@example.com')
      expect(result.attendees[0]).toContain('PARTSTAT=ACCEPTED')
      expect(result.attendees[0].find((item: string) => item.startsWith('CN='))).toBeUndefined()
    })
    ...
    it('should handle attendee with RSVP', () => {
      ...
      expect(result.attendees[0]).toContain('RSVP=TRUE')
      expect(result.attendees[0]).toContain('CUTYPE=INDIVIDUAL')
    })
