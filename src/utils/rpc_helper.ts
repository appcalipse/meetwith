...
const resolveENS = async (
  ...
  const avatar = avatarInfo
    ? (await getAvatar(address.toLowerCase(), avatarInfo!, provider))?.url
    : undefined
      ...
      switch (match[1]) {
        ...
        case 'erc1155': {
          ...
          if (match[1] === 'erc721') {
            ...
            const tokenOwner = provider.formatter.callAddress(
              await provider.call({
                to: addr,
                data: hexConcat(['0x6352211e', tokenId]),
              })
            )
            if (owner !== tokenOwner.toLowerCase()) {
              return undefined
            }
          ...
          if (
            ...
            (!metadata.image.match(/^https:\/\//i) &&
              !metadata.image.match(/^ipfs:\/\//i))
          ) {
            return undefined
          }
...
function _parseString(result: string): null | string {
  try {
    return toUtf8String(_parseBytes(result) as BytesLike)
  } catch (error) {}
