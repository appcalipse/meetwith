...
const loginWithWallet = async (): Promise<Account | undefined> => {
    ...
    try {
        ...
        return await createOrFetchAccount(accounts[0].toLowerCase(), dayjs.tz.guess())
    } catch(err) {
        return undefined
    }
}
...
const createOrFetchAccount = async (accountAddress: string, timezone: string): Promise<Account> => {
    ...
    try {
        ...
        if(e instanceof AccountNotFoundError) {
            ...
            const signature = await signDefaultMessage(accountAddress.toLowerCase(), nonce)
            account = await createAccount(accountAddress.toLowerCase(), signature, timezone, nonce)
        } else {
            throw e
        }
    }
